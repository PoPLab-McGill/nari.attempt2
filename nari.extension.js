    ...
    function stiatExtension(options) {
        ...
        var stiatObj = {
            ...
            category: {
                ...
                // Stimuli as words (no images here)
                media: [
                    { word: 'Tyron' },
                    { word: 'Malik' },
                    { word: 'Terrell' },
            ...
            trialsByBlock: [
                // Each object in this array defines a block
                {
                    ...
                    // In each block, we can include a number of mini-blocks, to reduce repetition of the same group/response.
            ...
            blockOrder: 'random',
            // Change to 'startRight' if you want to start with the category on the right in the first block.
            // Change to 'startLeft' if you want to start with the category on the left in the first block.
        ...
        return stiatObj; // Return the task object
    }
...
// Default feedback messages for each cutoff.
...
// Layout object for the trials where the category is on the left
var leftLayout = [
...
// Layout object for the trials where the category is on the right
var rightLayout = [
    {location:{left:6, top:1}, media:{word:piCurrent.leftKeyText}, css:piCurrent.keysCss},
    {location:{right:6, top:1}, media:{word:piCurrent.rightKeyText}, css:piCurrent.keysCss},
    {location:{left:6, top:4}, media:piCurrent.attribute1.title.media, css:piCurrent.attribute1.title.css},
...
API.addTrialSets('sort', {
    ...
    interactions: [
        ...
        // correct
        {
            conditions: [{ type: 'inputEqualsTrial', property: 'corResp' }], // Check if the input handle is equal to correct response
            actions: [
        ...
        // skip block
        {
            ...
            actions: [
                { type: 'setInput', input: { handle: 'skip2', on: 'enter' } } // Allow skipping if the next key is enter
            ]
        },
        // skip block
        {
...
for (var iBlock = 1; iBlock <= piCurrent.trialsByBlock.length; iBlock++) {
    ...
    if (piCurrent.trialsByBlock[iBlock-1].categoryTrials === 0) {
        // There are no category trials, so this is a practice block
        isPrac = true;
    ...
    if (iBlock === 2) {
        // Set the block2Condition variable
    ...
    // We separate each block into mini blocks to reduce repetition of categories and responses
    for (var iMini = 1; iMini <= piCurrent.trialsByBlock[iBlock-1].miniBlocks; iMini++) {
        ...
        if (!isPrac) { // If it is not a practice block
            // Add the category trials to the mixer's data
            mixer.data.push({
                mixer: 'repeat',
